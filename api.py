...
class SearchRequest(BaseModel):
    """Search by vector. Use num_candidates (e.g. 300) for general queries to improve recall."""
    vector: List[float]
    top_k: int = 10
...
def search_vectors(req: SearchRequest, x_api_key: Optional[str] = Header(None, alias="X-API-Key")):
    """Search for nearest vectors. Body: {"vector": [...], "top_k": 10, "num_candidates": 300}. Use num_candidates for general queries."""
    _require_api_key(x_api_key)
    try:
